...
if ($deleteId > 0) {
    if (!verifyCSRFToken($deleteToken)) {
        ...
        try {
            $stmt = $pdo->prepare('SELECT image_url, expiry_date FROM products WHERE id = :id');
            ...
            error_log('Expired product delete error: ' . $e->getMessage());
            $error = t('product_error');
        }
    }
}
...
// Get total count
$stmt = $pdo->query('SELECT COUNT(*) as total FROM products WHERE expiry_date IS NOT NULL AND expiry_date <= CURDATE()');
...
<head>
    ...
    <title><?= e(t('expired_products')) ?> - Bloom & Vine</title>
    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-x-hidden">
            ...
            <main class="flex-1 p-4 md:p-8">
                <!-- Header Section -->
                <div class="bg-gradient-to-r from-red-600 via-red-700 to-rose-800 text-white rounded-3xl p-8 mb-8 shadow-xl relative overflow-hidden">
                    <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div>
                            <h1 class="text-3xl md:text-4xl font-luxury font-bold mb-2">
                                <i class="fas fa-skull-crossbones me-3"></i><?= e(t('expired_products')) ?>
                            </h1>
                            <p class="text-red-200"><?= e(t('expired_products_desc')) ?></p>
                        </div>
                        <a href="products.php" 
                           class="inline-flex items-center gap-2 bg-white text-red-700 px-6 py-3 rounded-full hover:bg-red-50 transition-all duration-300 font-bold shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                ...
                <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                    <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                        <div>
                            <h2 class="text-lg font-bold text-gray-800"><i class="fas fa-list me-2 text-red-600"></i><?= e(t('expired_products')) ?></h2>
                        ...
                        <table class="min-w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <?php if (empty($products)): ?>
                                    <tr>
                                        <td colspan="7" class="px-6 py-8 text-center text-gray-400"><?= e(t('no_data_available')) ?></td>
                                    </tr>
                                <?php else: ?>
